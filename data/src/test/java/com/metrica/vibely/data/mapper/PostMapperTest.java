...
class PostMapperTest {
	...
	void toDTOTest() {
		...
		assertEquals(comments.stream().map(Post::getPostId).collect(Collectors.toSet()), postDTO.getComments());

		assertEquals(likedBy.stream().map(User::getUserId).collect(Collectors.toSet()), postDTO.getLikedBy());

		assertEquals(savedBy.stream().map(User::getUserId).collect(Collectors.toSet()), postDTO.getSavedBy());
